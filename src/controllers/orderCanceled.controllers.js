...
export const getOrderCanceled = async (req, res) => {
    ...
    try {
        ...
        const [resulActualizaStock] = await pool.query('UPDATE products SET stock = ? + ? WHERE id_product = ?', [resultOrderItem[0].cantidad,resultProduct[0].stock,resultOrderItem[0].id_product])
